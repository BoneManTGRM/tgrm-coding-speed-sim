/* TGRM simulator — with Events + postMessage for iframe host */
...
function params(){
  return {
    n:+els.n.value, err:+els.err.value, tau:+els.tau.value,
    fix:+els.fix.value, mc:+els.mc.value
  };
}
...
function run(){
  ...
  // 🔔 Also notify parent (for iframe host pages like index_dark.html)
